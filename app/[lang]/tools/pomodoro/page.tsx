   ...
   return (
      <div className="flex flex-col min-h-[80vh] w-full max-w-4xl mx-auto mb-12">
         <div className="mb-8 flex items-start mt-4">
            ...
            <div>
               <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 flex items-center text-transparent bg-clip-text bg-gradient-to-r from-rose-400 to-orange-400">
                  ...
                  {dict.dashboard.tools.zenPomodoro.title}
               </h1>
               <p className="text-slate-400 text-lg">
                  {dict.dashboard.tools.zenPomodoro.desc}
               </p>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full min-h-[500px]">
            ...
            {/* Timer Box */}
            <div className={`relative rounded-3xl p-8 lg:p-12 border overflow-hidden flex flex-col items-center justify-center shadow-2xl transition-colors duration-1000 ${mode === "work"
                  ? "bg-[#1f1118]/80 border-rose-500/20 shadow-rose-500/10"
                  : "bg-[#111f1c]/80 border-teal-500/20 shadow-teal-500/10"
               } backdrop-blur-xl`}>

               {/* Ambient Breathing Glow */}
               <div className={`absolute inset-0 opacity-20 filter blur-[80px] pointer-events-none rounded-full transition-colors duration-1000 ${isRunning ? "animate-pulse" : ""} ${mode === "work" ? "bg-rose-500" : "bg-teal-500"
                  }`}></div>
               ...
               <div className="relative z-10 flex bg-black/40 p-1 rounded-2xl mb-12 border border-white/5">
                  ...
                  >
                     ...
                     {dict.tools.pomodoro.breakMode}
                  </button>
               </div>

               <div className={`text-[6rem] sm:text-8xl md:text-[140px] font-black font-mono tracking-tighter transition-all mb-8 sm:mb-12 relative z-10 ${mode === "work" ? "text-rose-100" : "text-teal-100"
                  } drop-shadow-2xl`}>
                  {formatTime(timeLeft)}
               </div>
               ...
               <div className="flex gap-6 relative z-10">
                  <button
                     onClick={toggleTimer}
                     className={`w-20 h-20 rounded-full flex items-center justify-center transition-all shadow-xl hover:scale-105 active:scale-95 ${mode === "work"
                  ...
                  <div className="flex items-center justify-between mb-8 border-b border-white/5 pb-6">
                     <div>
                        <h3 className="text-xl font-bold text-white flex items-center">
                           {dict.tools.pomodoro.ambientOn}
                        </h3>
                        <p className="text-slate-400 text-sm mt-1">{dict.dashboard.tools.zenPomodoro.desc}</p>
                     </div>

                     <button
                        onClick={() => setSoundEnabled(!soundEnabled)}
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-xl active:scale-95 ${soundEnabled
