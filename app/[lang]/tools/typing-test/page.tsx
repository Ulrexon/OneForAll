    ...
    <div className="flex flex-col min-h-[80vh] w-full max-w-5xl mx-auto mb-12">
      <div className="mb-8 flex items-start mt-4">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 flex items-center text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">
            ...
            {dict.dashboard.tools.typingTest.title}
          </h1>
          <p className="text-slate-400 text-lg">
            {dict.dashboard.tools.typingTest.desc}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col-reverse lg:grid lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-1 gap-4">
          {/* Time Box */}
          <div className={`p-4 lg:p-6 rounded-3xl backdrop-blur-md shadow-2xl transition-all duration-300 border ${status === 'playing' ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-black/40 border-white/10'}`}>
            <div className="flex items-center text-slate-400 mb-1 lg:mb-2 font-medium uppercase tracking-wider text-[10px] lg:text-sm">
              <Timer className="w-3 h-3 lg:w-4 lg:h-4 mr-1 lg:mr-2" /> {dict.tools.typing.time}
            </div>
            <div className={`text-3xl lg:text-5xl font-mono font-bold ${timeLeft <= 10 && status === 'playing' ? 'text-red-400 animate-pulse' : 'text-white'}`}>
              {timeLeft}s
            </div>
          </div>

          {/* WPM Box */}
          <div className="p-4 lg:p-6 rounded-3xl bg-black/40 border border-white/10 backdrop-blur-md shadow-2xl">
            <div className="flex items-center text-slate-400 mb-1 lg:mb-2 font-medium uppercase tracking-wider text-[10px] lg:text-sm">
              <Zap className="w-3 h-3 lg:w-4 lg:h-4 mr-1 lg:mr-2 text-yellow-400" /> {dict.tools.typing.wpm}
            </div>
            <div className="text-3xl lg:text-5xl font-mono font-bold text-white">
          ...
          <div className="p-4 lg:p-6 rounded-3xl bg-black/40 border border-white/10 backdrop-blur-md shadow-2xl">
            <div className="flex items-center text-slate-400 mb-1 lg:mb-2 font-medium uppercase tracking-wider text-[10px] lg:text-sm">
              <Percent className="w-3 h-3 lg:w-4 lg:h-4 mr-1 lg:mr-2 text-blue-400" /> {dict.tools.typing.accuracy}
            </div>
            <div className="text-3xl lg:text-5xl font-mono font-bold text-white">
              {status === "finished" ? accuracy : (typed.length > 0 ? Math.round(
                (Array.from(typed).filter((char, i) => char === text[i]).length / typed.length) * 100
              ) : "--")}
              <span className="text-sm lg:text-2xl text-slate-500 ml-1">%</span>
            </div>
          </div>
          ...
          <button
            onClick={resetTest}
            tabIndex={-1}
            className="p-4 lg:p-4 rounded-3xl bg-white/5 border border-white/10 hover:bg-white/10 text-slate-300 font-medium transition-colors flex flex-col lg:flex-row items-center justify-center text-xs lg:text-base group"
          >
            <RotateCcw className="w-5 h-5 lg:mr-2 mb-1 lg:mb-0 group-hover:-rotate-90 transition-transform duration-500" />
            <span className="text-center">Restart <span className="hidden lg:inline">(Tab+Enter)</span></span>
          </button>
        </div>
        ...
        <div className="lg:col-span-3 flex flex-col h-full">
          <div
            ...
            className={`relative flex flex-col rounded-3xl bg-black/50 border backdrop-blur-xl overflow-hidden shadow-2xl min-h-[400px] p-8 lg:p-12 cursor-text transition-all duration-300 ${isFocused
                ...
                <div className="flex flex-col items-center justify-center h-full text-center space-y-4 sm:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
                  ...
                  <div className="flex gap-6 sm:gap-12">
                    <div className="flex flex-col items-center">
                      <span className="text-slate-400 text-xs sm:text-sm uppercase tracking-[0.2em] font-bold mb-1 sm:mb-3">WPM Speed</span>
                      <span className="text-5xl sm:text-7xl font-mono font-black text-white">{wpm}</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-slate-400 text-xs sm:text-sm uppercase tracking-[0.2em] font-bold mb-1 sm:mb-3">Accuracy</span>
                      <span className="text-5xl sm:text-7xl font-mono font-black text-white">{accuracy}<span className="text-3xl sm:text-4xl opacity-50">%</span></span>
                    </div>
                  </div>
                  <button
                    onClick={resetTest}
                    className="mt-4 sm:mt-6 px-8 py-4 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 rounded-2xl text-white font-bold text-lg shadow-[0_0_30px_rgba(16,185,129,0.3)] hover:shadow-[0_0_40px_rgba(16,185,129,0.5)] transition-all flex items-center group transform hover:-translate-y-1"
                  >
                    <RotateCcw className="w-5 h-5 mr-2 group-hover:-rotate-180 transition-transform duration-700" />
                    Play Again
                  </button>
                </div>
              ) : (
